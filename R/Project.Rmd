...
user1 <- user1[!(user1$state.x == "AK" & user1$state.x != "HI"),]
lessee1 <- user1[-owner_id,1]
lessee1 <- as.data.frame(lessee1)

day <- seq(as.Date('2018/01/01'), as.Date('2018/12/31'), by="day")


warehouse1 <- warehouse1 %>% slice(rep(1:n(), each = 365))

contract <-NULL
num_contracts <- sample(1:nrow(lessee),1)


for(i in 1:nrow(lessee1)){
  ...
  selected_warehouse <- warehouse[which(warehouse$storage_pref == lessee_pref),]
  ...
  selected_warehouse <-selected_warehouse[sample(nrow(selected_warehouse),1),]
  repeat{
    selected_warehouse <-selected_warehouse[sample(nrow(selected_warehouse),1),]
    if (num_skids < selected_warehouse$capacity){
      break
    }
  }
  capacity_check <- 
  ...
  selected_owner <- selected_warehouse[,13]
  status <- sample(c("Pending", "Accepted", "Declined", "Active", "Terminated" ),1)
 
  contract  <- data.frame(selected_id,selected_owner,selected_warehouse[,1],start_date, end_date, num_skids,lessee_pref, total_price, deposit, status)
  contract_final <- rbind(contract_final,contract)
}

colnames(contract_final) <- c("lessee_id", "owner_id", "wh_id", "start_date", "end_date", "num_skids", "storage_pref", "total_price", "deposit" , "status")

contract_final <- tibble::rowid_to_column(contract_final, "contract_id")

dbWriteTable(con, "Contract", contract_final, overwrite = TRUE)


contract_final <- read.csv("Contract.csv")
contract_final %>% group_by(wh_id) %>% summarise(sum(num_skids))

warehouse[which(contract_final %>% group_by(wh_id) %>% summarise(sum(num_skids)) > warehouse$capacity),]
```

```{r rating}
#Generates Weighted Average

terminated_contracts <- contract_final[which(contract_final$status == "Terminated"),] 
test <- terminated_contracts %>% group_by(wh_id) %>% count()
...
gen_rate <- function(n){
  ...
  rating_avg <- as.data.frame(rating_avg) 
  rating_avg <- rating_avg/n
  
  return(rating_avg)
}
  
test1 <- sapply(test$n, gen_rate)
test1 <- unlist(test1)
