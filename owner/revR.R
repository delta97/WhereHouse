...
a <- paste("SELECT weighted_average_rating FROM Warehouse WHERE owner_id=",test)
skids1 <- dbSendQuery(mydb, a)
skids <- fetch(skids1, n=-1)


#Create plot of number of skids rented 
png(filename="revPlot.png", width=500, height=500, type="cairo")
barplot(table(skids), xlab="Number of Skids", ylab = "Frequency of Rental")
#dev.copy(png, 'revPlot.png')
dev.off()
