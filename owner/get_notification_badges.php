<?php
	...
	$query = "SELECT * FROM Contract WHERE owner_id = $user_id AND status = 'Active'";
	$result = mysqli_query($connection, $query);
	$assoc_array = mysqli_fetch_assoc($result);
	$num_rentals = sizeof($assoc_array);

	//find out how many open requests there are
	$query2 = "SELECT * FROM Contract WHERE owner_id = $user_id AND status = 'Pending' OR status = 'Accepted'";
